...
class RelocationOptimizer:
    ...
    def __init__(self,
                 ...
                 traffic_flow_simulator: TrafficFlowSimulator,
                 verbose: bool = True):
        ...
        # computing probability origin destination matrix
        ...
        self.__possible_origins = [
            [[i for i in range(self.__zones_number) if self.__origin_destination_matrix[t, i, j] > 0] for j in
             range(self.__zones_number)] for t in range(self.__optimization_horizon + self.__look_ahead_horizon)]
        ...
        self.__possible_destinations_probabilities = [[[self.__origin_destination_probability[t, i, j] for j in
                                                      ...
                                                      range(self.__optimization_horizon + self.__look_ahead_horizon)]

        self.__start_zones = [[]] * (self.__optimization_horizon + self.__look_ahead_horizon)
        for time_frame in range(self.__optimization_horizon + self.__look_ahead_horizon):
            for i in range(self.__zones_number):
                self.__start_zones[time_frame].extend([i] * self.__outgoing_demand[time_frame][i])

        self.__trips = [[[]] * self.__zones_number] * (self.__optimization_horizon + self.__look_ahead_horizon)
        for time_frame in range(self.__optimization_horizon + self.__look_ahead_horizon):
            for i in range(self.__zones_number):
        ...
        self.__statistics = pd.DataFrame(index=list(range(self.__optimization_horizon)) + ['overall'],
                                         columns=['demand', 'objective', 'efficiency', 'efficiency_upper_bound',
                                                  'start_time', 'stop_time', 'elapsed_time',
                                                  'neighbourhood_search_perc_gain'], dtype=float)
        ...
        self.__statistics.at['overall', 'efficiency_upper_bound'] = 0
        self.__tempi = []

        return

    def set_optimization_procedure(self,
        ...
        self.__temperature = temperature_function
    ...
    def optimize(self):
        ...
        for t in range(self.__optimization_horizon):
            ...
            r = self.__optimization_round(starting_time_frame=t,
                                          initial_state=current_system_state)
            self.__relocation_solution.append(r)

            current_system_state, satisifed_demand = self.__time_frame_transition(s=current_system_state,
                                                                                  r=r,
                                                                                  time_frame=t)
        ...
        self.__statistics.at['overall', 'elapsed_time'] = self.__statistics.at['overall', 'stop_time'] - \
                                                          self.__statistics.at['overall', 'start_time']
        self.__statistics.at['overall', 'efficiency'] = self.__statistics.at['overall', 'objective'] / \
                                                        self.__statistics.at['overall', 'demand']
        self.__statistics.at['overall', 'efficiency_upper_bound'] = self.__statistics.at[
                                                                        'overall', 'efficiency_upper_bound'] / \
                                                                    self.__statistics.at['overall', 'demand']

        return

    def __compute_time_frame_statistics(self,
                                        time_frame: int,
    ...
    def __optimization_round(self,
                             starting_time_frame: int,
                             initial_state: list) -> list:

        r = self.__initial_optimization(starting_time_frame=starting_time_frame,
                                        initial_state=initial_state)

        obj = self.__compute_objective(r=r,
                                       initial_state=initial_state,
        ...
        if self.__perform_neighbourhood_search:
            r = self.__neighbourhood_search(r0=r,
                                            objective0=obj,
                                            initial_state=initial_state,
                                            starting_time_frame=starting_time_frame)
    ...
    def __initial_optimization(self,
        ...
        r, obj, first_tf_sat_demand = self.__lp_problem(starting_time_frame=starting_time_frame,
                                                        initial_state=initial_state)

        self.__statistics.at[starting_time_frame, 'efficiency_upper_bound'] = first_tf_sat_demand / np.sum(
            self.__outgoing_demand[starting_time_frame])
        self.__statistics.at['overall', 'efficiency_upper_bound'] += first_tf_sat_demand

        if self.__initial_optimization_method == 'LP+rounding':

            r = self.__relocation_rounding(r=r,
                                           initial_state=initial_state)

        elif self.__initial_optimization_method == 'MILP':

            r = self.__milp_problem(starting_time_frame=starting_time_frame,
                                    initial_state=initial_state)
    ...
    def __lp_problem(self,
                     starting_time_frame: int,
                     initial_state: list) -> (list, float, float):
        ...
        return [[r[t][i].value() for i in range(self.__zones_number)] for t in
                range(self.__look_ahead_horizon)], model.objective.value(), np.sum(
            [EO[0][i].value() for i in range(self.__zones_number)])

    def __milp_problem(self,
                       starting_time_frame: int,
                       initial_state: list,
                       fix_zero_relocation: list = None) -> list:
        ...
        for i in range(self.__zones_number):
            if (0, i) not in fix_zero_relocation:
                model += (-r[0][i] <= s[0][i], 'relocation_bound_' + str(i) + '_0')
        for i in range(self.__zones_number):
            for t in range(1, self.__look_ahead_horizon):
                model += (-r[t][i] <= s[t][i], 'relocation_bound_' + str(i) + '_' + str(t))
        ...
        return [[r[t][i].x if (t, i) not in fix_zero_relocation else r[t][i] for i in range(self.__zones_number)] for t
                in range(self.__look_ahead_horizon)]

    def __relocation_rounding(self,
                              r: list,
                              initial_state: list) -> list:
    ...
    def __neighbourhood_search(self,
                               r0: list,
                               objective0: float,
                               initial_state: list,
                               starting_time_frame: int) -> list:

        if self.__verbose:
            print('Starting neighbourhood search')

        incumbent_r = r0
        incumbent_objective = objective0

        iters = 0
        iters_no_improvement = 0

        time_frames = list(range(self.__look_ahead_horizon))  # TODO chidere se va bene la rimozione del time frame

        while iters < self.__max_iterations and iters_no_improvement < self.__max_iterations_no_improvement:

            t = random.sample(time_frames, k=1)[0]
            ...
            incumbent_r, incumbent_objective, improved = self.__time_frame_search(t=t,
                                                                                  incumbent_r=incumbent_r,
            ...
            if self.__verbose:
                ...
                if improved == 0:
                    ...
                    print(
                        f'Iteration {iters + 1} out of {self.__max_iterations} didn\'t change the objective')
                    print()

            iters += 1

        self.__statistics.at[starting_time_frame, 'neighbourhood_search_perc_gain'] = (
                (incumbent_objective - objective0) / objective0)
    ...
    def __time_frame_search(self, t: int, incumbent_r: list, incumbent_objective: int, initial_state: list,
        ...
        #     if new_objective > incumbent_objective:
        #
        #         return r_copy, new_objective, 0
        #
        #     elif new_objective < incumbent_objective:
        #         if np.random.binomial(1, self.__compute_acceptance_probability(iteration, new_objective,
        #                                                                        incumbent_objective)):
        #             return r_copy, new_objective, 1

        # indexes = list(map(None, *(iter(indexes),) * multiprocessing.cpu_count()))
        n_cpus = multiprocessing.cpu_count()
        indexes = list(chunks(indexes, n_cpus))
        list_size = len(indexes)

        for idx, index_slice in enumerate(indexes):

            procs = list()
            queue = multiprocessing.Queue()

            for index_couple in index_slice:
                proc = multiprocessing.Process(target=self.time_frame_search_parallelization,
                                               args=(index_couple, t, incumbent_r, incumbent_objective, initial_state,
                                                     starting_time_frame, iteration, queue))
            ...
            while not queue.empty():
                ...
                if self.__verbose:
                    print(
                        f'New optimum found after exploring {idx / list_size:.1%} of the neighbourhood ({(idx + 1) * n_cpus} neighbours)')

                return result[0], result[1], result[2]

        # for index_couple in indexes:
        #     start = time()
        #     results = self.time_frame_search_parallelization(indexes=index_couple, t=t, incumbent_r=incumbent_r,
        #                                                      incumbent_objective=incumbent_objective,
        #                                                      initial_state=initial_state,
        #                                                      starting_time_frame=starting_time_frame,
        #                                                      iteration=iteration)
        #     self.__tempi.append(time() - start)
        #     if results[2] != 2:
        #         return results
        # second_arg = (t, incumbent_r, incumbent_objective, initial_state, starting_time_frame, iteration)
        # results = pool.starmap(self.time_frame_search_parallelization, zip(indexes, repeat(second_arg)))

        return incumbent_r, incumbent_objective, 2

    def time_frame_search_parallelization(self, indexes: tuple, t: int, incumbent_r: list,
                                          incumbent_objective: int, initial_state: list, starting_time_frame: int,
                                          iteration: int, queue: multiprocessing.Queue = None):
        i, j = indexes
        r_copy = copy.deepcopy(incumbent_r)
        r_copy[t][i] += 1
        r_copy[t][j] -= 1
        ...
        if new_objective > incumbent_objective:
            ...
            # return r_copy, new_objective, 0
            queue.put((r_copy, new_objective, 0))

        elif new_objective < incumbent_objective and np.random.binomial(1,
                                                                        self.__compute_acceptance_probability(iteration,
                                                                                                              new_objective,
                                                                                                              incumbent_objective)):
            # return r_copy, new_objective, 1
            queue.put((r_copy, new_objective, 1))
        ...
        #     return np.exp(-(current_objective - new_objective) / (self.__d / np.log(iteration + 1e40)))

    def tempi(self):
        return self.__tempi

    def __compute_acceptance_probability(self,
                                         iteration: int,
                                         new_objective: float,
                                         current_objective: int) -> float:
        # print((current_objective - new_objective),
        #       np.exp(-(current_objective - new_objective) / (T0 - T0 * (iteration - T0) / (self.__max_iterations + 1 - T0))))
        # return np.exp(
        #     -(current_objective - new_objective) / (self.__T0 - self.__T0 * (iteration - self.__T0) / (self.__max_iterations + 1 - self.__T0)))
        return np.exp(
            -(current_objective - new_objective) / self.__temperature(iteration, self.__max_iterations))

    def __compute_objective(self,
                            r: list,
                            initial_state: list,
                            starting_time_frame: int) -> float:
    ...
    def __round_flows(self,
        ...
        if unbalance > 0:
            indexes = self.__get_rounding_indexes(EO, rounded_out, unbalance)
            ...
            indexes = self.__get_rounding_indexes(EI, rounded_in, unbalance)
    ...
    def __system_lp_problem(self,
                            r: list,
                            s: list,
                            time_frame: int,
                            starting_time_frame: int) -> (list, list):
        ...
        for i in range(self.__zones_number):
            m += (EI[i] == lpSum(
                EO[j] * self.__origin_destination_probability[starting_time_frame + time_frame][j, i] for j in
                self.__possible_origins[starting_time_frame + time_frame][i]),
                  'effective_incoming_outgoing_matching_' + str(i))
            m += (EO[i] <= s[i] + 0.5 * (EI[i] + r[i]), 'effective_outgoing_bound_' + str(i))
            m += (s[i] + EI[i] - EO[i] + r[i] >= 0, 'next_state_' + str(i))

        m.solve(PULP_CBC_CMD(msg=False))

        return [EO[i].value() for i in range(self.__zones_number)], [EI[i].value() for i in
                                                                     range(self.__zones_number)]

    def __monte_carlo_simulation(self,
                                 r: list,
                                 initial_state: list,
                                 starting_time_frame: int) -> float:

        total_objective = 0
        # TODO parallelizzare le simulazioni?
    ...
    def __single_tf_run(self,
                        s: list,
                        r: list,
                        time_frame: int) -> (list, list):

        s_r = [s[i] + min(0, r[i]) for i in range(self.__zones_number)]
        outgoing = [0] * self.__zones_number
        incoming = [0] * self.__zones_number

        random.shuffle(self.__start_zones[time_frame])

        for i in range(self.__zones_number):
            random.shuffle(self.__trips[time_frame][i])

        trips = copy.deepcopy(self.__trips[time_frame])

        start_zones = [self.__start_zones[time_frame][:int(len(self.__start_zones[time_frame]) / 2)],
                       self.__start_zones[time_frame][int(len(self.__start_zones[time_frame]) / 2):]]

        for t in range(2):

            s_r_copy = s_r.copy()

            for start_zone in start_zones[t]:
            ...
            if t == 0:
                s_r = [s_r[i] + max(0, r[i]) for i in range(self.__zones_number)]
    ...
    #         arrival_zone = np.random.choice(self.__possible_destinations[time_frame][start_zone], size=1,
    #                                         p=self.__possible_destinations_probabilities[time_frame][start_zone])
    #
    #         if s_r[start_zone] > 0:
    ...
    #
    #     for t in range(2):
    #         for start_zone, arrival_zone in flows[t]:
    #
    #             if s_r[start_zone] > 0:
    #                 s_r[start_zone] -= 1
    #                 s_r[arrival_zone] += 1
    ...
    def __time_frame_transition(self,
                                s: list,
                                r: list,
                                time_frame: int):
    ...
    def plot_relocation(self,
        ...
        df = pd.DataFrame(
            data=[[t, i, self.__relocation_solution[t][i], self.__relocation_solution[t][i] > 0] for i in
                  range(self.__zones_number) for t in
    ...
    def get_overall_statistics(self):
        return self.__statistics[['efficiency', 'efficiency_upper_bound', 'elapsed_time']].loc[
            'overall']  # access by adding ['efficiency']

    def get_statistics(self):
        return self.__statistics[
            ['efficiency', 'efficiency_upper_bound', 'neighbourhood_search_perc_gain', 'elapsed_time']]
